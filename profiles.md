...
| 求职 | qingdao-ai-resume、job-scout、EDUQA-from-pdfs、lark 办公簇、browser-act |
...
- 槽位只覆盖 claude / codex；opencode 不参与 skill 配置。
- `--profile 开发` 的 `[slot-only]` 项（如 lark 簇）是「快照有、live 无」——
  若该场景用不到，`slot remove`；若要用，用户确认后 `dispatch --enable` 双 app。
